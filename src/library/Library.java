...
public class Library {
	...
	// add librarian
	public boolean addLibrarian(String f, String l, Address addr, String birthdate, String hireDate, Double salary,
			EarnedDegree degree) {
		...
		for (int c = 0; c < people.size(); c++) {
			...
			if (people.get(c) instanceof Employee) {
				...
				if (((Employee) people.get(c)).getEmployeeID() == empID) {
					...
					((Employee) people.get(c)).completeTimeSheet(hours);// type cast cause it is part of array of people
																		// (polymorph)
		...
		for (int c = 0; c < people.size(); c++) {
			...
			if (people.get(c) instanceof Librarian) {
				...
				if (((Librarian) people.get(c)).getEmployeeID() == empID) {
					...
					return ((Librarian) people.get(c)).getTotalPay(); // type cast cause it is part of array of people
																		// (polymorph)
	...
	public double getTotalWages() {
		...
		for (int c = 0; c < people.size(); c++) {
			...
			if (people.get(c) instanceof Librarian) {
				...
				// for this year divided by 12 and times the amound of complete months that have
				// passed
	...
	public String toString() {
		...
		// run through array of people. will autocall toString based on what type of
		// person it is
		output.append("\n\nPeople:");
		for (Person pers : people) {
